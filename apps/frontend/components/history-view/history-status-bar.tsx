...
import { summarizeWorkingHours, type WorkingHours } from '@/lib/working-hours';
        ...
        {multiSelectMode && selectedHashesSize > 0 ? (
          <>
            ...
            <Tooltip
              label={
                workingHoursSummary
                  ? `稼働時間内のみに分布: ${workingHoursSummary}`
                  : '稼働時間制約なし (Settings から有効化可能)'
              }
              disabled={selectedHashesSize < 2}
            >
              <Button
                size="xs"
                variant="filled"
                ...
                <Popover.Dropdown>
                  <Stack gap="xs">
                    <Switch
                      ...
                      checked={autoFetch}
                      onChange={(e) =>
                        onAutoFetchToggle(e.currentTarget.checked)
                      }
                    />
                    <Divider />
